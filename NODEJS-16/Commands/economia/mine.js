...
async execute (client, message, args){
  ...
  if(cooldown.has(message.author.id)){
    message.channel.send(`${message.author}, espera 6 segundos antes de volver a usar el comando`)
  ...
  if(picos < 1){
    return message.channel.send("Se necesita un `Pico` para poder usar este comando `Para comprarlo use $shop transfiera el dinero que vale el Pico a efectivo y despues use $buy Pico`")
  }
